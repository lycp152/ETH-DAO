...
describe("ETH-DAO test", function () {
  ...
  const demoAlchemyRPCEndpoint = "https://ethereum-sepolia.publicnode.com";

  const sdk = new ThirdwebSDK(
    new ethers.Wallet(
      demoWallet.privateKey,
      new ethers.providers.JsonRpcProvider(demoAlchemyRPCEndpoint)
    )
  );
  ...
  it("token contract is deployed", async function () {
    ...
    // トークンのシンボルがMICOYとなっているか確認する
    assert.equal(tokenInfo.symbol, "MICOY");
  });
  ...
  it("vote contract has right info", async function () {
    ...
    assert.equal(metadata.name, "micoy Fashion DAO Governance");
  });
